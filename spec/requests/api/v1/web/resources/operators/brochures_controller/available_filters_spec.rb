...
describe Api::V1::Web::Resources::Operators::BrochuresController, type: :request do
  subject(:get_filters) do
    get(available_filters_api_web_resources_operators_brochures_path)
  end

  describe 'GET /api/web/resources/operators/brochures/available_filters' do
    ...
    context 'when user has operator role' do
      ...
      it 'returns correct operator filters' do
        expect(parsed_json['response']['available_filters']['role_based_available_filters']).to be_blank
      end
    end
  end
end
